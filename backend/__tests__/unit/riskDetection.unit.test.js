...
describe('Risk Detection Service - Unit Tests', () => {
  let testSchool;
  let testUser;
  let testStudent;
  let testSettings;
  ...
  beforeEach(async () => {
    ...
    // Wait a bit and verify school was created
    await new Promise(resolve => setTimeout(resolve, 100));
    const verifySchool = await School.findById(testSchool._id);
    if (!verifySchool) {
      // Retry school creation
      testSchool = await School.create({
        name: `Test School Retry ${timestamp}`,
        district: 'Kigali',
        sector: 'Nyarugenge',
        isActive: true
      });
  ...
  describe('checkAttendanceRisk - Unit Tests', () => {
  ...
  describe('checkPerformanceRisk - Unit Tests', () => {
  ...
  describe('checkSocioeconomicRisk - Unit Tests', () => {
